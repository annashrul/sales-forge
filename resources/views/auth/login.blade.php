<x-guest-layout>
    ...
    <div class="mb-6">
        <div class="text-[10px] uppercase tracking-[0.22em] text-zinc-500 font-bold mb-1.5">Welcome back</div>
        <h1 class="text-balance text-2xl sm:text-3xl font-black tracking-[-0.025em] leading-[1.05] text-zinc-950">
            Sign in to keep
            <span class="italic font-serif font-light text-[#FF5A36]">drafting.</span>
        </h1>
        <p class="mt-2 text-sm text-zinc-500">Pick up where you left off.</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input id="email" name="email" type="email" value="{{ old('email') }}" required autofocus autocomplete="username" placeholder="you@company.com"
        ...
        <div x-data="{ show: false }">
            <div class="flex items-center justify-between mb-2">
                <label for="password" class="block text-xs font-bold uppercase tracking-wider text-zinc-700">Password <span class="text-[#FF5A36]">*</span></label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-xs font-bold text-[#FF5A36] hover:text-[#C44425] transition">Forgot?</a>
                @endif
            </div>
            <div class="relative">
                <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z"/></svg>
                <input id="password" name="password" :type="show ? 'text' : 'password'" required autocomplete="current-password" placeholder="Your password"
        ...
        <label for="remember_me" class="inline-flex items-center select-none cursor-pointer pt-1">
            <input id="remember_me" type="checkbox" class="rounded border-zinc-300 text-[#FF5A36] shadow-sm focus:ring-[#FF5A36]/30" name="remember">
            <span class="ms-2 text-sm text-zinc-700">Keep me signed in</span>
        </label>
    ...
    <p class="mt-4 text-center text-sm text-zinc-500">
        New here?
        <a href="{{ route('register') }}" class="font-bold text-[#FF5A36] hover:text-[#C44425] transition">Create a free account</a>
    </p>
</x-guest-layout>
